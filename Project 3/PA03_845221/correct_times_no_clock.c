...
//// I would  mark myself down if I got this code without any preface or reasoning for why the times are the way they are ////////
...
void printerFunct(float currentTime, char* str1, char* str2, PCB currentProcess, int processCounter, config cycles);
...
///////////////// functions ////////////////////////////////////////////////////////////////////////////////
/* runTimeCalc() - This function is a swtich which calculates the runTime in miliseconds for a specific task and returns it*/
int runtTimeCalc(tasks *currentTask, config cycles){
    int runTime; 
...
/* simulation() - The heavy lifting function. This bad boy does it all. It takes in the process queue, config structure 
...
*/
    ...
    outputOp(cycles, "- Simulator program starting", currentTime, ' ');
    ...
    while(emptyProcess(processQ) != 0)
    {
            ...
            printerFunct(currentTime,  "Ready","- OS: preparing process", currentProcess, processCounter, cycles);
            printerFunct(currentTime,  "Running","- OS: starting process", currentProcess, processCounter, cycles);
       ...
       while(emptyTask(&currentProcess.taskQ) != 0){
            ...
            outputProcess(cycles, "- Process", ": start", currentTask.taskSpec, currentTask.taskID , currentTime, processCounter);
            ...
            //output time change 
            outputProcess(cycles, "- Process", ": end", currentTask.taskSpec, currentTask.taskID , currentTime, processCounter);
        }

        //terminate process and increment the process counter 
        printerFunct(currentTime,  "Terminated","- OS: removing process", currentProcess, processCounter, cycles);
        processCounter++;
    ...
    outputOp(cycles, "- Simulator program ending", currentTime, ' ');
...
void printerFunct(float currentTime, char* str1, char* str2, PCB currentProcess, int processCounter, config cycles){
    ...
    outputOp(cycles, str2, currentTime, processCounter);
}
