...
void logToMonitor(tasks currentTask, clock_t startTime, threadStruct threadParam, config cycles, Queue *processQ);
void logToFile(FILE *filePointer, tasks currentTask, clock_t startTime, threadStruct threadParam, config cycles, Queue *processQ);
void logToBoth(FILE *filePointer, tasks currentTask, clock_t startTime, threadStruct threadParam, config cycles, Queue *processQ);
...
*/
    if(cycles.logMethod == 'M')
  	  logToMonitor(currentTask, startTime, threadParam, cycles, processQ);
  	...
  	if(cycles.logMethod == 'F' || cycles.logMethod == 'B'){
		...
		if(cycles.logMethod == 'F')
  			logToFile(filePointer, currentTask, startTime, threadParam, cycles, processQ); 
  		else
  			logToBoth(filePointer, currentTask, startTime, threadParam, cycles, processQ);
  		fclose(filePointer);
...
void logToMonitor(tasks currentTask, clock_t startTime, threadStruct threadParam, config cycles, Queue *processQ){
	...
	printf("%f - Simulator program starting\n", (float)(startTime - startTime)/CLOCKS_PER_SEC);
	currentTime = clock();
	...
	while(stopFlag == 1) 
		{
			...
			if(currentTask.taskID == "application")
				processCounter++; 
			currentTime = clock();
			printf("%f - Process %d: start %s %s \n", (float)(currentTime - startTime)/CLOCKS_PER_SEC, processCounter, currentTask.taskSpec, currentTask.taskID);
	...
	while(stopFlag == 1) 
		{
			...
			if(currentTask.taskID == "application")
				processCounter++; 
			time(&currentTime);
			fprintf(filePointer, "%f - Process %d: start %s %s \n", (float)(currentTime - startTime), processCounter, currentTask.taskSpec, currentTask.taskID);
